...
ggplot() +
  ...
  geom_point(data = bird_data_plot |>
             ...
             aes(x = year,y = n_id), 
             stat = "identity") +
  geom_text(data = bird_data_plot |>
               group_by(year) |> 
               summarise(n_id = n()), 
             aes(x = year, y = n_id, label = n_id), 
             vjust = -0.5) +
...
bird_data_year<- rename(bird_data_year, `Weißsterniges Blaukehlchen` = Weißsterniges.Blaukehlchen)
```

## total number of individuals per species

```{r}
bird_sum <- colSums(bird_data_year[,-1], na.rm = TRUE)
```

## total number of individuals per species > 10

```{r}
bird_bigger_ten <- names(bird_sum[bird_sum>10])
```

## number of individuals per species per year [but more than 10i total] - plot

```{r}
bird_data_year_long <- 
  bird_data |> filter(`Artbezeichnung deutsch` %in% bird_bigger_ten) |>
  group_by(`Artbezeichnung deutsch`, year) |>
  summarise(n = n()) |>
  data.frame()

bird_data_year_bigger_ten <- 
  bird_data_year_long |> filter(Artbezeichnung.deutsch %in% bird_bigger_ten)|>
  data.frame()

ggplot() +
  geom_bar(data = bird_data_year_long, aes(x = year, y = n,  fill = Artbezeichnung.deutsch),
           stat = "identity") +
scale_fill_manual(values = rainbow(142, s=.6, v=.9)[sample(1:142,142)]) +
...
```{r}
data_plot <- bird_data  |> 
         filter(`Artbezeichnung deutsch` %in% 'Amsel') |> 
...
Phi.weight <- list(formula =  ~ Geschlecht) #response ~ predictor
