...
#-> needs fix bc of messed up order of species in data_cmr when temp added
...
fit.birds.cjs.models <- function(){
  ...
  #Phi.tmp <- list(formula =  ~ Temp)
  
  p.tmp <- list(formula =  ~ Temp)
...
#saveRDS(birds_cjs_models, file = "data_brandenburg/output/birds_cjs_models_temp_phi.rds")
#saveRDS(birds_cjs_models, file = "data_brandenburg/output/birds_cjs_models_temp_p.rds")

birds_cjs_models <- readRDS("data_brandenburg/output/birds_cjs_models_temp_phi.rds")
birds_cjs_models <- readRDS("data_brandenburg/output/birds_cjs_models_temp_p.rds")
...
for (i in seq(1,14)){
  birds_cjs_models[[i]]$Phi.tmp.p.dot$results$reals$Phi$Temp <- 
    as.character(birds_cjs_models[[i]]$Phi.tmp.p.dot$results$reals$Phi$Temp)}
for (i in seq(1,14)){
  birds_cjs_models[[i]]$Phi.tmp.p.dot$results$reals$Phi$Temp <- 
    as.numeric(birds_cjs_models[[i]]$Phi.tmp.p.dot$results$reals$Phi$Temp)}


## Plot Phi for Temp

ggplot_phi_temp <- list()
for (i in seq(1,14)){ggplot_phi_temp[[i]] <- 
  ggplot(birds_cjs_models[[i]]$Phi.tmp.p.dot$results$reals$Phi,
  ...
  labs(x = "Temperature\n", 
       y = "Phi\n",
       subtitle= element_text(birds_cjs_models[[i]]$Phi.tmp.p.dot$data$data$Artbezeichnung.deutsch[1]))}
ggplot_phi_temp[[1]]$labels<-
  labs(x = "Temperature\n", 
       y = "Phi\n",
       subtitle= element_text(birds_cjs_models[[1]]$Phi.tmp.p.dot$data$data$Artbezeichnung.deutsch[1]),
       title = "Model: Phi.temp.p.dot")
combined_plot <- wrap_plots(ggplot_phi_temp, nrow = 4, byrow=FALSE)
ggsave("ggplot_phi_temp.png", plot = combined_plot, 
...
for (i in seq(1,14)){
  birds_cjs_models[[i]]$Phi.dot.p.tmp$results$reals$p$Temp <- 
    as.character(birds_cjs_models[[i]]$Phi.dot.p.tmp$results$reals$p$Temp)}
for (i in seq(1,14)){
  birds_cjs_models[[i]]$Phi.dot.p.tmp$results$reals$p$Temp <- 
    as.numeric(birds_cjs_models[[i]]$Phi.dot.p.tmp$results$reals$p$Temp)}


ggplot_p_temp <- list()
for (i in seq(1,14)){ggplot_p_temp[[i]] <-
  ggplot(birds_cjs_models[[i]]$Phi.dot.p.tmp$results$reals$p, 
         aes(Temp, estimate, ymin = lcl, ymax = ucl)) +
  geom_ribbon(alpha=0.2) +
  geom_point() +
  geom_line() +
  labs(x = "Temperature\n", 
       y = "p\n",
       subtitle= element_text(birds_cjs_models[[i]]$Phi.dot.p.tmp$data$data$Artbezeichnung.deutsch[1]))}
ggplot_p_temp[[1]]$labels<-
  labs(x = "Temperature\n", 
       y = "p\n",
       subtitle= element_text(birds_cjs_models[[1]]$Phi.dot.p.tmp$data$data$Artbezeichnung.deutsch[1]),
       title = "Model: Phi.dot.p.temp")
combined_plot <- wrap_plots(ggplot_p_temp, nrow = 4, byrow=FALSE)
ggsave("ggplot_p_temp.png", plot = combined_plot, 
       path = "data_brandenburg/plots", 
       width = 13, height = 13, units = "in", dpi = 300)
