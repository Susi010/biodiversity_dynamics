...
names(starlings)[names(starlings) =="...1"] <- "ID" # rename to ID column
...
```{r}
# your code in here
star_rural <- subset(starlings,starlings$habitat=="rural")
star_urban <- subset(starlings,starlings$habitat=="urban")

cjs_rur <- crm(data = star_rural)
cjs_urb <- crm(data = star_urban)

cjs_rur
cjs_urb

cjs_rur_inv <- cjs.hessian(cjs_rur)
cjs_urb_inv <- cjs.hessian(cjs_urb)

inverse_logit(cjs_rur$results$beta$Phi)
inverse_logit(cjs_rur$results$beta$p)

inverse_logit(cjs_urb$results$beta$Phi)
inverse_logit(cjs_urb$results$beta$p)
...
View(starlings_cjs_models[["model.table"]])

View(starlings_cjs_models[["Phi.sex.time.p.dot"]])
predict(starlings_cjs_models[["Phi.sex.time.p.dot"]])
...
If infected -> recapture higher?

```{r}
starlings_nona <- na.omit(starlings)

star_ninf <- subset(starlings_nona,starlings_nona$infected=="No")
star_inf <- subset(starlings_nona,starlings_nona$infected=="Yes")

cjs_ninf <- crm(star_ninf)
cjs_inf <- crm(data = star_inf)

cjs_ninf
cjs_inf

cjs_ninf_inv <- cjs.hessian(cjs_ninf)
cjs_inf_inv <- cjs.hessian(cjs_inf)

inverse_logit(cjs_ninf_inv$results$beta$Phi)
inverse_logit(cjs_ninf_inv$results$beta$p)

inverse_logit(cjs_inf_inv$results$beta$Phi)
inverse_logit(cjs_inf_inv$results$beta$p)

cjs_ninf_inv_h <- cjs.hessian(cjs_ninf_inv)
predict(object = cjs_ninf_inv_h,
        newdata = data.frame(sex = c("Female", "Male")),
        se = TRUE)

cjs_inf_inv_h <- cjs.hessian(cjs_inf_inv)
predict(object = cjs_inf_inv_h,
        newdata = data.frame(sex = c("Female", "Male")),
        se = TRUE)
```
...
Ex_m2_proc <- process.data(data = starlings_nona,groups = c("sex","habitat","infected"))
Ex_m2_design <- make.design.data(data = Ex_m2_proc)


phi_dot <- list(formula = ~ 1)
phi_inf <- list(formula = ~ infected)
p_inf <- list(formula = ~ infected)
...
Heat_wave <- matrix(rep(c(0, 1, 1, 0, 0, 0), each = nrow(starlings_nona)),
                    ncol =6)
colnames(Heat_wave) = paste("Heat_wave", 1:6, sep = "")
starlings_nona <- cbind(starlings_nona, Heat_wave)
starlings_nona$weight <- round(rnorm(nrow(starlings_nona), 80, 3))

Ex_m3_proc <- process.data(starlings_nona)

Ex_m3_design.Phi <-
  list(static = c("weight", "sex","infected","habitat"),
       time.varying = c("Heat_wave"))

Ex_m3_design.p <- list(static = c("habitat","infected"))

Ex_m3_design.parameters <- list(Phi = Ex_m3_design.Phi, p = Ex_m3_design.p)

Ex_m3_design <- make.design.data(data = Ex_m3_proc,
                              parameters = Ex_m3_design.parameters)

Phi.heat.sex.habitat <- list(formula =  ~ Heat_wave * sex * habitat)

p.infected.habitat <- list(formula =  ~ c(infected,habitat))
